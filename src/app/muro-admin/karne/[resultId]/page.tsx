...
import { CheckCircle2, XCircle, MinusCircle, FileText, Target, BookOpen, Clock, Activity, Home, Trophy, PlayCircle, ChevronLeft } from "lucide-react";
import Link from "next/link";

export default async function AdminKarneViewPage({ params }: { params: Promise<{ resultId: string }> }) {
  ...
  if (!result) {
    return <div className="p-10 text-slate-800 text-center mt-20">Karne bulunamadı.</div>;
  }
  ...
  const realRank = scoredResults.findIndex(r => r.userId === result.userId) + 1;
  ...
  return (
    <div className="font-sans text-slate-800 pb-20">
      <div className="max-w-6xl mx-auto px-4 pt-4">
         <Link href={`/muro-admin/sinavlar/${result.examId}`} className="inline-flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 text-slate-600 rounded-xl hover:bg-slate-50 transition-colors shadow-sm mb-4">
           <ChevronLeft className="w-4 h-4" /> Sınava Geri Dön
         </Link>
      </div>

      <main className="max-w-6xl mx-auto space-y-8 px-4">
        {/* Başlık Kartı */}
        <div className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm relative overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 relative z-10">
            <div>
              <span className="px-3 py-1 bg-emerald-50 text-emerald-600 border border-emerald-100 rounded-lg text-[10px] font-bold tracking-widest uppercase mb-4 inline-block">Yönetici Karne Görüntüleyicisi</span>
              <h2 className="text-3xl font-black text-slate-900">{result.user.name} - Karne</h2>
              <div className="flex items-center gap-4 mt-3">
                 <p className="text-slate-500 max-w-xl text-sm font-bold">{result.user.email} | {result.exam.title}</p>
